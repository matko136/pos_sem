...
writeAndReadSocketCipher(unsigned long long int *input, int serv_address, bool connectS, int *destSourcSock) {
...
int odosliPoziadavku(int poziadavka, ZDIEL *zdiel, char *sprava, int cisloKl, int cisloVlak, int cisloSpr) {
    ...
    if (poziadavka == 1) {
        ...
        int count = 0;
        char vlakno[256];
        klData->pocetVlakien = 0;
        while (!strcmpareCipher("69", vlakno, buffer, sockfd)) {
            unsigned long long int *genKeys = generujKluceRSA();
            char buffer[256];
            write(*sockfd, genKeys, 2 * sizeof(unsigned long long int));
            unsigned long long int *sifrkey[256];
            char keyy[256];
            bzero(sifrkey, 256 * sizeof(unsigned long long int));
            read(*sockfd, sifrkey, 256 * sizeof(unsigned long long int));
            char *tempPrijKey = desifruj(sifrkey, genKeys[2], genKeys[1]);
            strcpy(keyy, tempPrijKey);
            free(tempPrijKey);
            int key = atoi(keyy);
